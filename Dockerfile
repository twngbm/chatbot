...
RUN apt update && apt install -y python3.7 python3-pip python3.7-dev gcc --no-install-recommends \
    ...
    && python3.7 -m pip install numpy ckiptagger==0.1.1 tensorflow==1.15.2 PyJWT==1.7.1 python-Levenshtein==0.12.0 websockets==8.1 fuzzywuzzy ckipnlp nest_asyncio requests --no-cache-dir \
